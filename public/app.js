...
function renderDiscoveryPlan() {
  ...
  if (statusEl) {
    statusEl.textContent = state.discoveryBusy ? "Running broker sweep and web search\u2026" : !plan.canAutoDiscover ? "Add profile URLs below, then tap Discover listings." : "";
  }
}
...
function renderFindings() {
  ...
  if (hint) {
    hint.textContent = pending > 0 ? "Yes = yours \xB7 Not me = skip" : confirmed > 0 ? "Queued for removal after you approve." : "Tap Discover listings to search, or paste URLs you already know.";
  }
  renderDiscoveryPlan();
  ...
  if (list) {
    ...
    }).join("") : `<div class="empty">No links yet. Tap <strong>Discover listings</strong> above to search brokers and the web, or paste URLs you already know.</div>`;
  }
...
function renderWalletModal() {
  ...
  const liveHint = state.walletConfig?.liveEnabled ? "Sepolia Smart Account upgrade uses MetaMask wallet_sendCalls (EIP-5792)." : "Smart Account records EIP-7702 + ERC-7715 permissions. Enable WALLET_LIVE_MODE for Sepolia on-chain upgrade.";
  body.innerHTML = `
    ${state.walletAddress ? `
...
function hasEntitledPayment(mode) {
  ...
  return sessions.some(
    (session) => session.mode === mode && (session.status === "paid" || session.status === "authorized")
  );
}
...
function renderSubscriptionUpsell() {
  ...
  if (!banner) return;
  const show = Boolean(state.currentCaseId && state.currentStatus) && (state.aiEntitlement?.mode === "one-off" || hasEntitledPayment("one-off")) && !hasSubscriptionEntitlement() && !isUpsellDismissed(state.currentCaseId);
  banner.hidden = !show;
}
async function ensureCasePayment(options = {}) {
  ...
  if (statusEl) {
    statusEl.textContent = hasEntitledPayment(mode) ? `${paymentPlanLabel(mode)} confirmed \u2014 agent AI unlocked for this case.` : "Payment session prepared. Open Settings \u2192 Payment rails if MetaMask did not confirm.";
  }
  if (!options.quiet) {
    addChat(
      "agent",
      hasEntitledPayment(mode) ? `${paymentPlanLabel(mode)} is set for this case. I'll still pause for your approval before anything sends.` : "Finish payment in Settings \u2192 Payment rails if MetaMask did not confirm."
    );
  }
  renderSubscriptionUpsell();
...
function renderVaultPanel() {
  ...
  if (passphraseInput) {
    ...
    passphraseInput.placeholder = hasVaultKey ? "Only needed to wrap the vault key" : "Unavailable after refresh \u2014 export redacted data without a passphrase";
  }
}
...
function setupDelegates() {
  ...
  document.addEventListener("click", (e) => {
    ...
    if (e.target.closest("#upsell-subscribe")) {
      e.preventDefault();
      preparePayment("subscription").then(() => addChat("agent", "Weekly monitor prepared. Check Settings \u2192 Payment rails for status.")).catch(write);
      return;
    }
