...
test("discoverExposureCandidates uses ephemeral search labels for broker sweep queries", async () => {
  ...
  try {
    ...
    assert.ok(queries.some((item) => /John Smith New York, NY site:/i.test(item)));
    assert.ok(!queries.some((item) => /J\.S\./i.test(item)));
  } finally {
...
test("discoverExposureCandidates uses Brave results when configured", async () => {
  ...
  try {
    ...
    assert.ok(!exposures.some((item) => item.sourceUrl.includes("example.com")));
  } finally {
    globalThis.fetch = originalFetch;
    if (originalBraveKey === undefined) delete process.env.BRAVE_SEARCH_API_KEY;
    else process.env.BRAVE_SEARCH_API_KEY = originalBraveKey;
...
test("describeDiscoveryPlan explains broker sweep and manual fallback", () => {
  ...
  assert.equal(manual.canAutoDiscover, true);
  assert.equal(manual.methods.some((item) => item.id === "broker-sweep"), true);
