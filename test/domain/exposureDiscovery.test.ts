...
test("fetchWebSearchCandidates prefers Venice augment/search when configured", async () => {
...
test("fetchWebSearchCandidates falls back to Brave when Venice search fails", async () => {
  process.env.VENICE_API_KEY = "venice-test";
...
test("describeDiscoveryPlan explains broker sweep and manual fallback", () => {
  ...
  assert.match(manual.methods.find((item) => item.id === "manual-only")?.detail || "", /VENICE_API_KEY/);
